...
def index(request):
    ...
    user_following_list.append(user_profile.user)
    for username in user_following_list[::-1]:
        feed_list = Post.objects.filter(user=username).order_by('-created_at')
...
def signin(request):
    if request.method=="POST":
        ...
        if user is not None:
            ...
            messages.info(request,'User does not Exists / or passwords do not match')
    ...
    if request.method=="POST":
        ...
        if 'image_upload' in request.FILES:
            ...
            if content_type.split('/')[0] == 'image':
                ...
                video = request.FILES.get('image_upload')
                caption = request.POST['caption']
